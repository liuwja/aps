...
(function($){
	$.fn.extend({
		pagination: function(opts){
			...
			return this.each(function(){
				...
				$this.html(DWZ.frag["pagination"].replaceAll("#pageNumFrag#", pageNumFrag).replaceAll("#totalPageNum#", pc.getTotalPageNum()).replaceAll("#currentPage#", pc.getCurrentPage())).find("li").hoverClass();
				...
				if (pc.hasPrev()){
					...
					_bindEvent($prev, pc.getCurrentPage()-1, pc.targetType(), pc.rel(), "prev");
					_bindEvent($first, 1, pc.targetType(), pc.rel(), "first");
				} else {
				...
				if (pc.hasNext()) {
					...
					_bindEvent($next, pc.getCurrentPage()+1, pc.targetType(), pc.rel(), "next");
					_bindEvent($last, pc.numPages(), pc.targetType(), pc.rel(), "last");
				} else {
					$next.add($last).addClass("disabled").find(">a").hide();
				}
	
				$this.find(setting.nums$).each(function(i){
					_bindEvent($(this), i+interval.start, pc.targetType(), pc.rel(), "num");
				});
			...
			function _bindEvent($target, pageNum, targetType, rel, direction){
				$target.bind("click", {pageNum:pageNum, direction:direction}, function(event){
					dwzPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum, direction:event.data.direction}});
	...
	$.extend(Pagination.prototype, {
		...
		getTotalPageNum:function(){
			var pageNum = Math.ceil(parseInt(this.opts.totalCount)/parseInt(this.opts.numPerPage)); 
