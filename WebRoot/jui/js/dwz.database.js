...
//				}
				for (var key in args) {
					...
					var id =  _util.lookupId(key);
					//compare by id first. by denolin
...
//					}
					$.bringBack(args);
					if (typeof(memberSelctCallBack) == "function") memberSelctCallBack(args);
				});
			});
		},
		suggest: function(){
		...
		itemDetail: function(){
			...
			function initSuffix($tbody) {
				$tbody.find('>tr').each(function(i){
					$(':input, a.btnLook, a.btnAttach', this).each(function(){
						...
						txtID =  $this.attr('id')
						if(txtID)$this.attr('id', txtID.replaceSuffix(i));
			...
			function tdHtml(field){
				...
				if (field.name.endsWith("_#index#")) suffix = "_#index#";
				...
				switch(field.type){
					...
					case 'lookup':
						...
						html = '<input type="hidden" id="'+field.lookupGroup+'_'+field.lookupPk+'" name="'+field.lookupPk+suffix+'"/>'
							+ '<input type="text" id="'+field.lookupGroup+'_'+field.name+'" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'"/>'
