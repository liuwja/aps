...
<table class="table" width="450%" layoutH="120">
<thead>
<tr>
	<th></th>
...
<c:if test="${ o.performanceType eq '指标型' and o.indexType eq '望大型'}">
...
<c:choose>
	...
	<c:otherwise>
		...
		<td style="background-color:${mon.trColor1}">${mon.createUser2}</td>
		<td style="background-color:${mon.trColor1}"><fmt:formatDate value="${mon.createTime2}" pattern="yyyy-MM-dd "/></td>
	</c:otherwise>
</c:choose>									
				</c:if>
			</c:forEach>
...
<c:if test="${ o.performanceType eq '指标型' and o.indexType eq '望小型'}">
<tr target="key" rel="${o.id}">
	<td>
		<input type="radio" group="code" name="key" value="${o.id}">
	</td>
				...
				<c:if test="${ mon.monthIndex == monthIndex3}">
					<c:choose>
						...
						<c:otherwise>
							...
							<td style="background-color:${mon.trColor2}">${mon.createUser2}</td>
							<td style="background-color:${mon.trColor2}"><fmt:formatDate value="${mon.createTime2}" pattern="yyyy-MM-dd "/></td>
						</c:otherwise>
					</c:choose>	
				</c:if>
			</c:forEach>
...
<c:if test="${ o.performanceType eq '指标型' and o.indexType eq '望目型'}">
<tr target="key" rel="${o.id}">
	<td>
		<input type="radio" group="code" name="key" value="${o.id}">
	</td>
				...
				<c:if test="${ mon.monthIndex == monthIndex3}">
					<c:choose>
						...
						<c:otherwise>
							...
							<td style="background-color:${mon.trColor3}">${mon.createUser2}</td>
							<td style="background-color:${mon.trColor3}"><fmt:formatDate value="${mon.createTime2}" pattern="yyyy-MM-dd "/></td>
						</c:otherwise>
					</c:choose>
				</c:if>
			</c:forEach>
...
<c:if test="${ o.performanceType eq '项目型'}">
<tr target="key" rel="${o.id}">
	<td>
		<input type="radio" group="code" name="key" value="${o.id}">
	</td>
	...
	<c:if test="${ fn:length(o.monthList) != 0 }">
		<c:forEach var="monthIndex3" begin="1" end="12" step="1">
			...
			<c:forEach items="${o.monthList }" var="mon" begin="0" end="11" step="1">
				<c:if test="${ mon.monthIndex == monthIndex3}">
					<c:set var="flag" value="true"></c:set>
					<c:if test="${ mon.checkResult eq '已达标'  }">
						<c:choose>
							<c:when test="${performanceIndex.statisticsType eq '当月实际' }">
								<td style="background-color:${mon.trColor1}"><fmt:formatDate value="${mon.myMonth}" pattern="yyyy-MM"/></td>
								<td style="background-color:${mon.trColor1}">${mon.monTargetValue}</td>
					...
					<c:if test="${ mon.checkResult eq '未达标'  }">
						<c:choose>
							<c:when test="${performanceIndex.statisticsType eq '当月实际' }">
								<td style="background-color:${mon.trColor1}"><fmt:formatDate value="${mon.myMonth}" pattern="yyyy-MM"/></td>
								<td style="background-color:${mon.trColor1}">${mon.monTargetValue}</td>
...
<c:if test="${ o.performanceType eq '其它' }">
<tr target="key" rel="${o.id}">
	<td>
		<input type="radio" group="code" name="key" value="${o.id}">
	</td>
