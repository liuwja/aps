	...
	<c:set var="len" value="${fn:length(o.monthList) == 0 ? 1 : fn:length(o.monthList)+1}"></c:set>
		<tr target="key" rel="${o.id}">
			...
			<c:forEach items="${o.monthList }" var="mon">
				...
				<c:if test="${ o.indexType eq '望大型'  }">
					<tr style="background-color: ${ mon.trColor1  }">
					<td>
					<fmt:formatDate value="${mon.myMonth}" pattern="yyyy-MM"/></td>
					<td>${mon.monTargetValue}</td>
					<td>${mon.monRealityTargetValue}</td>
					</tr>									
				</c:if>	
				<c:if test="${ o.indexType eq '望小型'  }">
					<tr style="background-color: ${ mon.trColor2  }">
					<td>
					<fmt:formatDate value="${mon.myMonth}" pattern="yyyy-MM"/></td>
					<td>${mon.monTargetValue}</td>
					<td>${mon.monRealityTargetValue}</td>
				...
				<c:if test="${ o.indexType eq '望目型'  }">
					<tr style="background-color: ${ mon.trColor3  }">
					<td>
					<fmt:formatDate value="${mon.myMonth}" pattern="yyyy-MM"/></td>
					<td>${mon.monTargetValue}</td>
					<td>${mon.monRealityTargetValue}</td>
				...
				<c:if test="${ o.indexType ne '望大型' and o.indexType ne '望小型' and o.indexType ne '望目型' }">
