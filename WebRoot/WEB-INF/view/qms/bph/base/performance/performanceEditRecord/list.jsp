...
function stampingDaliy_checkData(){
	...
	$("#performanceEditRecordList").submit();
}
</script>
<div class="pageHeader">
<form id="performanceEditRecordList" onsubmit="return navTabSearch(this);" rel="pagerForm" action="ptm/performanceEditRecord/list.do" method="post">
<div class="searchBar">
	<table class="searchContent">
		...
		<tr>
			...
			<td>
				<input type="search" name="performanceTargetClass" value="${param.performanceTargetClass }"/>
			</td>	
			<td>
				指标内容： 
			</td>
			...
			<td>
				<input type="search" name="indexType" value="${param.indexType }"/>
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input type="search" name="updateUser" value="${param.updateUser }"/>
			</td>	
			<td>
				修改原因： 
			</td>
			...
			<td>
				<select name="operatorModule" id="operatorModule" onchange="">
					<option value="">请选择</option>
					<option value="绩效指标设定" <c:if test="${log.operatorModule eq '绩效指标设定' }">selected="selected"</c:if>>绩效指标设定</option>
					<option value="年度指标设定" <c:if test="${log.operatorModule eq '年度指标设定' }">selected="selected"</c:if>>年度指标设定</option>
					<option value="月度指标设定" <c:if test="${log.operatorModule eq '月度指标设定' }">selected="selected"</c:if>>月度指标设定</option>
					<option value="月度指标结果设定" <c:if test="${log.operatorModule eq '月度指标结果设定' }">selected="selected"</c:if>>月度指标结果设定</option>
				</select>
			</td>		
			<td>
