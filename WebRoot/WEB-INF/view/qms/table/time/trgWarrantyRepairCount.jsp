...
function checktwrcSigleCount() {
    ...
    if(matrixTableType == 'Inverted' ){
    	$("#trgWarrantyRepairCount").attr("action","warrantyRepair/trgDownWarrantyRepairCount.do");
    }else{
    	$("#trgWarrantyRepairCount").attr("action","warrantyRepair/trgWarrantyRepairCount.do");
    }
    var curFormDom = $("#trgWarrantyRepairCount", navTab.getCurrentPanel());
	curFormDom.submit();
	return true;
}		
</script>
<div class="pageHeader" style="position:static">
	<form onsubmit="return navTabSearch(this);" id="trgWarrantyRepairCount" rel="pagerForm" action="warrantyRepair/trgWarrantyRepairCount.do" method="post">
	<div class="searchBar">
		<table class="searchContent">
			<tr>
                <td>机型类别：</td>
                <td>
					<select name="productType" onchange="loadProductData('#prodFamilyTmList1', '#trginsCountpartTypeList1');">
    			...
    			<td>产品系列：</td>
                <td>
                	<div id="prodFamilyTmList1" class="dropdownlist"></div>
                </td>	
				<td>故障小类：</td>
    			<td>
    				<input type="hidden" name="faultReasonID" id="twrcfr_id" value="${vo.faultReasonID}"/>
    				<input type="hidden" name="faultReasonCode" id="twrcfr_code" value="${vo.faultReasonCode}"/>
    				<input type="text" name="faultReasonTxt" id="twrcfr_name" size="12" readonly="true" value="${vo.faultReasonTxt}" style="float: left;"/>    				
    				<a id="btn" style="display:inline-block" onclick="faultReasonSel(this, 'twrcfr')" class="btnLook btn" width=950 height=500 lookupGroup="twrcfr">故障小类选择</a>  
					<a style="display:inline-block;" class="btnClear" href="javascript:void(0);" onclick="clearFault('twrcfr')"  title="清空"></a> 
    			</td>
    			<td>故障小类是否有效：</td>
    			<td>
					...
					<div class="buttonActive"><div class="buttonContent"><button type="button" onclick="checktwrcSigleCount();">查找</button></div></div>
    			</td>
    			<td>
    				<div class="buttonActive"><div class="buttonContent"><button type="button" onclick="getDataSourceByMenuName('市场质量统计分析', '具体分析', '保内维修分析(正三角)', '三角阵');">数据来源</button></div></div>
    			</td>
			</tr>
			<tr>
				<td>统计方式：</td>                       
                ...
                <td>型号：</td>
				<td>
					<div id="trginsCountpartTypeList1" class="dropdownlist"></div>
				</td>
    			<td>故障大类：</td>
    			<td>
    				<input type="hidden" name="faultTypeID" id="twrfr_id" readonly="true" value=""/>  
                    <input type="hidden" name="faultTypeCode" id="twrfr_code" readonly="true" value="${vo.faultTypeCode}"/>  
                    <input type="text" name="faultTypeTxt" id="twrfr_name" size="12" readonly="true" value="${vo.faultTypeTxt}" style="float: left;"/>                  
                    <a onclick="faultTypeSel(this, 'twrfr');" style="display:inline-block;"  id="btn" class="btnLook btn" width=950 height=500 lookupGroup="twrfr">故障大类选择</a>
                    <a style="display:inline-block;"  class="btnClear" href="javascript:void(0);" onclick="clearFault('twrfr')" title="清空"></a> 
    			</td>
    			<td>百台内：</td>
				<td>
					...
					<div class="buttonActive"><div class="buttonContent"><button type="button" onclick="exportExcelByCommon('warrantyRepair/excelOutput_trgWarrantyRepairCount.do', '#trgWarrantyRepairCount');">导出</button></div></div>
				</td>
    		</tr>
    		<tr>
    			<td>统计数据：</td>   
				...
				<td>
					...
					<div id="trginsCountRegionList1" class="dropdownlist"></div>
				</td>
...
<!-- 						<c:if test="${fn:length(o.repairPercent)<fn:length(list[0].repairPercent)}"> -->
<!-- 							<td></td> -->
<!-- 						</c:if> -->
					</c:if>
...
<!-- 						<c:if test="${fn:length(o.reCount)<fn:length(list[0].reCount)}"> -->
<!-- 							<td></td> -->
<!-- 						</c:if> -->
					</c:if>
				</tr>		
...
<script type="text/javascript">
	$(function(){
		loadProductFamilyData("#prodFamilyTmList1", "productFamilyTxt", [${vo.productFamilyTxt}], ${jsonProFamily});
    	loadProductTypeData("#trginsCountpartTypeList1", "partTypeListTxt", [${vo.partTypeListTxt}], ${jsonParts});
     	loadRegionData("#trginsCountRegionList1", "regionListTxt", [${vo.regionListTxt}], ${jsonRegions});
	});
